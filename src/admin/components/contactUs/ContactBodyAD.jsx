...
export default function ContactBodyAD({ msg }) {
    return (
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar flex flex-col gap-4 bg-white dark:bg-gray-800">
            {/* User Message */}
            <div className="w-full">
                <div className="bg-gray-100 dark:bg-gray-700/50 p-4 rounded-2xl rounded-tl-none text-sm text-gray-700 dark:text-gray-200 leading-relaxed shadow-sm relative border border-gray-100 dark:border-gray-600">
                    <p className="whitespace-pre-wrap">{msg.message || msg.content || "No message content."}</p>
                </div>
                {msg.createdAt && (
                    <div className="flex items-center justify-between mt-1.5 pl-2">
                        ...
                        {!msg.isRead && <span className="text-[10px] font-medium text-indigo-500">New Message</span>}
                    </div>
                )}
            </div>
